...
**Matomo n’est pas encore actif sur le site.** L’instance `https://verbox.matomo.cloud/`, l’identifiant de site `1` et le script `https://cdn.matomo.cloud/verbox.matomo.cloud/matomo.js` sont renseignés dans `site.config.json`. Le mode CNIL de cette instance a été enregistré et vérifié ; `cnilConfigurationVerified` est à `true`. Le nom de l’éditeur reste à compléter : `audience.enabled` reste à `false`. La confidentialité indique cet état réel.
