...
**Matomo n’est pas encore actif.** L’éditeur a fourni l’instance `https://verbox.matomo.cloud/`, l’identifiant de site `1` et le script `https://cdn.matomo.cloud/verbox.matomo.cloud/matomo.js`. Ces valeurs sont renseignées dans `site.config.json`. Le nom de l’éditeur et la vérification des paramètres d’exemption CNIL côté Matomo restent à compléter : `audience.enabled` et `cnilConfigurationVerified` restent donc à `false`. La confidentialité indique cet état réel.
