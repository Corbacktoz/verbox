...
**Matomo n’est pas encore actif.** Aucun compte ou serveur Matomo n’a été fourni. `audience.enabled` et `cnilConfigurationVerified` restent à `false`, et aucune adresse fictive n’est appelée. La confidentialité indique cet état réel. La suppression de Google peut être publiée indépendamment de l’activation de Matomo.

## Raccordement sans serveur personnel

1. Créer une instance [Matomo Cloud](https://matomo.org/matomo-cloud/) pour `https://verbox.fr`. Le service hébergé est payant après l’essai : voir les [tarifs](https://matomo.org/pricing/). Ne pas souscrire un abonnement sans choix explicite de l’éditeur. L’alternative est une instance Matomo maintenue sur un hébergement PHP/MySQL.
...
4. Régler la suppression automatique des données détaillées à **90 jours maximum** et des statistiques agrégées à **25 mois maximum**, conformément au texte du site. Vérifier les logs de l’hébergeur Matomo, le contrat de sous-traitance, les lieux de traitement, les sous-traitants et les garanties de transfert le cas échéant. Matomo Cloud annonce un hébergement à Francfort ; vérifier le contrat en vigueur et compléter l’information publique avec le prestataire retenu.
5. Compléter `privacy.editorName` dans `site.config.json`. L’utilisateur a fourni uniquement l’adresse publique `corback.inc@gmail.com` : aucun nom légal n’a été inventé. Compléter séparément les mentions légales applicables au statut de l’éditeur et documenter l’intérêt légitime, notamment pour le public mineur. La politique proposée est une base technique, pas une attestation de conformité RGPD complète.
