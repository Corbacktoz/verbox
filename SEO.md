...
## Mise à jour du 20 septembre 2026 — Résolution de l’Audit

- **Périmètre d’indexation :** Le sitemap comprend désormais **24 URL indexables** (accueil, 3 niveaux scolaires CE2/CM1/CM2, hub des fiches `/fiches/`, 6 leçons par temps, hub des verbes `/conjugaison/`, 10 fiches de verbes dédiées `/conjugaison/:verbe/`, aide `/aide/`, à propos `/a-propos/`). Les pages privées ou techniques (`/progres/`, `/mentions-legales/`, `/confidentialite/`, `/404.html`) restent en `noindex`.
- **En-têtes de sécurité et hébergement :** GitHub Pages n’interprétant pas nativement les directives de fichiers `_headers`, la politique de sécurité en production sur GitHub Pages est assurée par la balise `<meta http-equiv="Content-Security-Policy">` avec `upgrade-insecure-requests;`. Le fichier `_headers` et `server.js` fournissent les en-têtes HTTP stricts (`Strict-Transport-Security`, `Permissions-Policy`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`) pour les hébergements avec reverse-proxy ou serveur d’application Node.
- **Accessibilité & Ergonomie :** Restauration du focus clavier après quiz, piégeage du focus dans les modales (`dialog`), suppression des boutons inertes (son, vestiaire, 0 points) sur les pages statiques sans JavaScript interactif, et suppression totale du débordement horizontal sur mobiles (320px, 375px, 390px) via le passage à la ligne du pied de page (`flex-wrap: wrap`) et le confinement (`overflow-x: clip`).
- **Rigueur pédagogique :** Validation des accords du participe passé avec l’auxiliaire *être* (féminin, pluriel, politesse) et de la variante *« puis »*, classification précise des groupes de verbes (`aller` au 3e groupe), et rappels de conjugaison contextualisés.
